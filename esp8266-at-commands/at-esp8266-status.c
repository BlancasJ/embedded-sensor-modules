...
void send_at_command(char *answer_esp, int16 wait_time, int8 lenghtstr)
{
   int8 i=0;
   ...
   //printf(lcd_putc,"\fesp: %s"answer_esp);
}
...
void main()
{
   ...
   {
      printf("AT");
      send_at_command(answer,3,3);
      printf(lcd_putc,"\fAT: %s",answer);
      delay_ms(1000);
   }
}

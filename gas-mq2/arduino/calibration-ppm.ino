...
{
  return ( ((float)RL_VALOR*(4095.0-raw_adc)/raw_adc));
}
...
float Calibracion(float mq_pin){
  ...
  val = val/50.0;                                                                                         //calcular el valor medio
  val = val/RAL;
  return val;
}
...
{
   if ( gas_id == GAS_LP ) {
     return porcentaje_gas_(rs_ro_ratio,LPCurve);
   }
  return 0;
}
 
int porcentaje_gas_(float rs_ro_ratio, float *pcurve)
{
